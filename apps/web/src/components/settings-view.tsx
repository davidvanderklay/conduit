...
function PlaybackSettings({ preferences, update }: PreferencePageProps) {
  return <div className="grid items-start gap-7 2xl:grid-cols-2">
    <div className="space-y-7"><SettingsGroup title="PLAYER"><SettingRow icon={PlayCircle} title="Resume behavior" description="What to do when opening something partially watched"><Select value={preferences.resumeBehavior} onChange={(value) => update("resumeBehavior", value as DevicePreferences["resumeBehavior"])} options={[["ask", "Ask every time"], ["always", "Always resume"], ["restart", "Start over"]]} /></SettingRow><Divider /><RangeRow label="Default volume" description="Starting player volume" value={preferences.volume} min={0} max={100} suffix="%" onChange={(value) => update("volume", value)} /></SettingsGroup><SettingsGroup title="AUTOPLAY"><SettingToggle label="Autoplay next episode" description="Continue after the next-episode prompt." checked={preferences.autoplay} onChange={(value) => update("autoplay", value)} /></SettingsGroup></div>
