...
export function ElectronPlayerOverlay({ initialTitle }: { initialTitle: string }) {
  ...
  return (
    ...
    >
      {/* Stremio-style edge scrims for contrast on bright scenes - taller to cover fullscreen */}
      <div
      ...
      />
      <div
        className={`pointer-events-none absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-black via-black/80 to-transparent transition-opacity duration-300 ${controlsVisible ? "opacity-100" : "opacity-0"}`}
        aria-hidden="true"
      />
      <div
        className={
